...
Edge :: Edge(int node1, int node2, float wt): node_start(node1), node_end(node2), weight(wt){};
...
//     float weight;
//     Edge(){}
//     Edge(int node1, int node2, float wt): node_start(node1), node_end(node2), weight(wt){}
// };
...
void Graph :: KruskalMST (std :: pair< std :: vector<int>, std :: vector<int> >& result) {
    ...
    for (auto& e : edgelist) {
        ...
        if (root1 != root2) {
           // result.push_back(e);
           (std::get<0>(result)).push_back(e.node_start);
           (std::get<1>(result)).push_back(e.node_end);
...
std::pair< std :: vector<int>, std :: vector<int> > MST(std::vector<std::vector<float>> distances) {
    ...
    for (int i = 0; i < num_nodes; i++) {
        for (int j = 0; j < num_nodes; j++) {
            if (i < j) {
              // std :: cout << "\nnum_nodes : " << distances[i][j] << std :: endl;
              Edge e1(i, j, distances[i][j]);
              g1.AddEdge(e1);
            }
        }
    }
